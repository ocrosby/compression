...
/*
 * Huffman has legitimate implementation freedom in tree tie-breaking
 * and bit-packing order, so we test round-trip behavior rather than
...
static void assert_round_trip(const uint8_t *in, size_t in_len) {
    ...
    size_t n_enc = huffman_encode(in, in_len, enc, sizeof enc);
    CTP_ASSERT(n_enc != HUFFMAN_ERROR);

    size_t n_dec = huffman_decode(enc, n_enc, dec, sizeof dec);
    CTP_ASSERT_EQ_BYTES(dec, n_dec, in, in_len);
}

static void test_encode_empty(void) {
    uint8_t out[16];
    size_t n = huffman_encode(NULL, 0, out, sizeof out);
    CTP_ASSERT_EQ_INT(n, 0);
}

static void test_decode_empty(void) {
    uint8_t out[16];
    size_t n = huffman_decode(NULL, 0, out, sizeof out);
    CTP_ASSERT_EQ_INT(n, 0);
}
...
static void test_encode_output_buffer_too_small(void) {
    uint8_t in[] = {'A', 'B', 'C'};
    uint8_t out[1];
    size_t n = huffman_encode(in, sizeof in, out, sizeof out);
    CTP_ASSERT_EQ_INT(n, HUFFMAN_ERROR);
}
...
static void test_decode_output_buffer_too_small(void) {
    ...
    size_t n_enc = huffman_encode(in, sizeof in, enc, sizeof enc);
    CTP_ASSERT(n_enc != HUFFMAN_ERROR);

    size_t n_dec = huffman_decode(enc, n_enc, dec, sizeof dec);
    CTP_ASSERT_EQ_INT(n_dec, HUFFMAN_ERROR);
}

static void test_decode_rejects_truncated_input(void) {
    uint8_t in[] = {0x01, 0x02};
    uint8_t out[16];
    size_t n = huffman_decode(in, sizeof in, out, sizeof out);
    CTP_ASSERT_EQ_INT(n, HUFFMAN_ERROR);
}
